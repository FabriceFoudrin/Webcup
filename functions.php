...
function createInvoice($product, $price){
    ...
    $status = -1;
    $ip = getIp();
    $sql = "INSERT INTO `invoices` (`code`, `address`, `price`, `status`, `idforfaitcapsule`,`ip`)
...
function getProduct($id){
    global $conn;
    $sql = "SELECT * FROM `forfaits_capsules` WHERE `idforfaitcapsule`='$id'";
    $result = mysqli_query($conn, $sql);
    while($row = mysqli_fetch_assoc($result)){
        return $row['libellecapsule'];
    }
}
...
function getDescription($product){
    global $conn;
    $sql = "SELECT * FROM `forfaits_capsules` WHERE `idforfaitcapsule`='$product'";
    $result = mysqli_query($conn, $sql);
